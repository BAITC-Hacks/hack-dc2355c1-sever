...
### Задержка
Замеры с ноутбука в Алматы до OpenAI API (сеть — основная часть времени; на сервере ближе к API меньше):

| этап | медиана |
|---|---|
| триаж | < 1 мс |
| роутер (до решения) | ~1.0 с |
| исполнитель (с действиями) | 1.5 – 3.5 с |
| TTS до первого байта | ~1.3 с |

Все этапы видны в трассировке каждой реплики (`stages_ms`) и в панели супервизора (p50/p95).
...
- **Задержка.** Полный ответ сейчас 3–4.5 с, что выше ориентира ТЗ в 1.5 с. Узкое место — сетевые вызовы LLM; роутер укладывается примерно в 1 с. Следующие шаги: стрим ответа исполнителя в TTS по предложениям, быстрый путь без инструментов для `fast_path_eligible`-сценариев.
