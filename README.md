...
STT — ElevenLabs Scribe (fallback — OpenAI `gpt-4o-transcribe` с подсказкой про русский и казахский; `whisper-1` казахский пишет латиницей), TTS — ElevenLabs Flash со стримом (fallback — OpenAI TTS, затем браузерный `speechSynthesis`). Провайдеры переключаются через `.env` — демо не падает, если один API недоступен.
