...
Нужны ключи OpenAI (обязательно) и ElevenLabs (голос; у ключа нужны права Text to Speech и Speech to Text — без них работают OpenAI `gpt-4o-transcribe` и браузерный синтез).
...
STT — OpenAI `gpt-4o-transcribe` с подсказкой «только русский или казахский» (запасной — ElevenLabs Scribe с `language_code=kaz`: без подсказки Scribe принимает казахский за турецкий). Поддерживаются только русский и казахский: если распознанный текст почти без кириллицы (арабское письмо, турецкая латиница), реплика распознаётся повторно другим провайдером. TTS — ElevenLabs Flash со стримом; если ElevenLabs ответил ошибкой (ключ без прав, лимит), озвучивает OpenAI TTS, затем браузерный `speechSynthesis`. Провайдеры переключаются через `.env` — демо не замолкает, если один API недоступен.
...
- **STT.** Качество распознавания казахской и смешанной речи определяет всё остальное. Запасной `gpt-4o-transcribe` с подсказкой пишет казахский кириллицей почти без ошибок, `whisper-1` — нет.
