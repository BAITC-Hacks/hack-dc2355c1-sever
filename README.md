...
STT — OpenAI `gpt-4o-transcribe` с подсказкой про русский и казахский; запасной — ElevenLabs Scribe с `language_code=kaz` (без подсказки Scribe принимает казахский за турецкий). На смешанной речи `gpt-4o-transcribe` точнее, по скорости они близки (0.7–1.5 с на реплику); основной выбирается `STT_PROVIDER`, TTS — ElevenLabs Flash со стримом (fallback — OpenAI TTS, затем браузерный `speechSynthesis`). Провайдеры переключаются через `.env` — демо не падает, если один API недоступен.
