...
| TTS | ElevenLabs `eleven_flash_v2_5` со стримом, голос Kari (русскоязычный, из библиотеки ElevenLabs) — основной; OpenAI `tts-1` со стримом — запасной или для казахских ответов (`TTS_PROVIDER_KK`); браузерный `speechSynthesis` |
...
| `TTS_PROVIDER_KK` | пусто | отдельный провайдер для казахских ответов: `openai` разборчивее, но до звука дольше |
| `ELEVENLABS_VOICE_ID` | `Jbte7ht1CqapnZvc4KpK` (Kari) | голос ElevenLabs |
...
- **Казахское произношение.** Голосов носителей казахского в библиотеке ElevenLabs нет; используется русскоязычный голос Kari. Замер (4 фразы × 3 повтора, обратное распознавание, доля верных ключевых слов): англоязычный голос по умолчанию — 83 %, Kari — 89 %, OpenAI `tts-1` — 94 % при +~1.2 с до звука. Длинные числа (годы) ElevenLabs иногда произносит неверно.
