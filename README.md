...
**Разговор**
- Голос через микрофон в браузере и текст как резервный канал.
...
6. **TTS → динамик.** Каждое готовое предложение ответа сразу озвучивается (ElevenLabs, запасной — OpenAI TTS) и играет в браузере потоком.
...
| Опционально | NVIDIA API (OpenAI-совместимый) — быстрый путь роутера; OpenAI `text-embedding-3-small` — сужение кандидатов. Оба выключены по умолчанию |
...
.venv/bin/python -m scripts.eval_router --mode strong
...
# Смоук-тест API (27 проверок) на запущенном сервере — локальном или задеплоенном
...
| смоук-тест API (`smoke_api`) | 27 из 27, локально и на Railway |
...
| NVIDIA API | быстрый путь роутера | нет, выключен |

**Интерфейсы приложения:**

| Метод | Путь | Назначение |
|---|---|---|
| WS | `/ws/session` | реплика голосом (бинарное аудио) или текстом `{"type":"text","text":"…"}` → `transcript`, `bot_partial`, mp3-чанки, `trace`, `final` |
...
- **Быстрый путь NVIDIA** реализован в каскаде, но выключен: бесплатный endpoint `llama-3.1-8b-instruct` помечен deprecated, а маленькие открытые модели слабо понимают казахский. Метрика «Быстрый маршрут» в панели супервизора поэтому показывает 0 %.
