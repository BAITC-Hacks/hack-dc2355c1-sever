...
Замер по WebSocket с ноутбука в Алматы до OpenAI и ElevenLabs, ответ со звуком ElevenLabs, реплики разных типов (справка, данные клиента, казахский, вне тематики, прощание):

| | медиана | максимум |
|---|---|---|
| до первого звука ответа | **2.2 с** | 2.4 с |
| роутер (до решения) | ~1.0 с | |
| TTS: от первого предложения до первого байта звука | ~0.4 с | |
...
- **без пауз при действиях**: если исполнителю нужны действия (поиск клиента, проверка платежа), клиент сразу слышит «Сейчас проверю», пока они выполняются;
- **системные ответы без LLM**: прощание, «вне тематики», переспрос — по шаблонам кита сразу после роутера (~1.3 с до звука).
...
- **Задержка.** Медиана до первого звука — 2.2 с: ниже 3 с, но выше ориентира ТЗ в 1.5 с. Узкое место — два последовательных сетевых вызова LLM (роутер ~1 с + первый токен исполнителя). Дальше: быстрый путь без инструментов для `fast_path_eligible`-сценариев, сервер ближе к API, realtime-STT со стримом.
