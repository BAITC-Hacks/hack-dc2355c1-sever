...
window.voiceOrb = (() => {
  ...
  for (const [name, token] of [["baseColor1", "--brand"], ["baseColor2", "--brand-accent"], ["baseColor3", "--ink-strong"]]) {
    const hex = palette.getPropertyValue(token).trim().slice(1);
    gl.uniform3f(gl.getUniformLocation(program, name), ...[0, 2, 4].map(i => parseInt(hex.slice(i, i + 2), 16) / 255));
  }
